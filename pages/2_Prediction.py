...
if st.session_state.pred_df is not None and st.session_state.master_df is not None:
    ...
    st.subheader("Grafik Aktual")
    actual_tail = master_df["harga_emas_antam_idr"].tail(60).reset_index()

    fig1 = px.line(
        actual_tail,
        x="date",
        y="harga_emas_antam_idr",
        title="harga emas antam aktual (60 hari terakhir)",
        labels={"harga_emas_antam_idr": "harga (idr)", "date": "tanggal"},
    )
    fig1.update_layout(showlegend=False)
    st.plotly_chart(fig1, use_container_width=True)

    st.subheader(f"Grafik Prediksi Forward ({horizon} Hari ke Depan)")
    fig2 = go.Figure()
    fig2.add_trace(
        go.Scatter(
            x=pred_df["pred_date"],
    ...
    fig2.add_trace(
        go.Scatter(
            ...
            mode="lines+markers",
            name="hybrid lightgbm",
        )
    )
    fig2.update_layout(
        title="hasil forecasting",
        xaxis_title="tanggal prediksi",
    ...
    if st.session_state.zip_bytes is None:
        try:
            actual_plot_bytes = fig_to_png_bytes(fig1)
            forecast_plot_bytes = fig_to_png_bytes(fig2)
